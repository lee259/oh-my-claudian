...
- Expose Low, Medium, and High as the initial fallback for enabled models. Prefer session-independent native queries for model reasoning metadata, with real ACP sessions as a discovery fallback; never create a chat session solely for discovery. Persist and prefer the chosen model's advertised reasoning metadata, and prune reasoning state when a model is disabled.
