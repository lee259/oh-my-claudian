...
export class MessageRenderer {
  ...
  appendInterruptIndicator(contentEl: HTMLElement): void {
    ...
    textEl.createSpan({
      ...
      text: '\u00B7 What should Oh My Claudian do instead?',
    });
  }
  ...
  private showRewindMenu(
    ...
    // Keyboard activation has no pointer coordinates; anchor the menu to the
    // button rect instead so the menu is reachable without a mouse.
    if (event.detail > 0) {
